...
module.exports = function (req, res, next) {
    if (!req.body || !req.body.map
        || !req.body.map_coordinate_x || !req.body.map_coordinate_y
        || !req.body.user_id) {
        res.status(400).json({ error: "Missing arguments" });
        return;
    }
    doQuery("UPDATE `users` SET `map` = ?, `map_coordinate_x` = ?, `map_coordinate_y` = ? WHERE `id` = ?",
        [req.body.map, req.body.map_coordinate_x, req.body.map_coordinate_y, req.body.user_id]).then(function (result) {
